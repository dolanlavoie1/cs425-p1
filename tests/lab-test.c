...
// Read bytes from a fake server transport.
...
// Write bytes to a fake server transport.
...
// Test the pure helper functions that do not require a transport.
...
// Test reading lines and replies from a fake transport.
...
// Test error handling for reading lines and replies.
...
// Test writing data and sending commands through a fake transport.
void test_write_and_command_errors(void)
{
  ...
  TEST_ASSERT_EQUAL_INT(-1, smtp_send_command(&transport, "x", 250, reply,
                                              sizeof(reply)));
  server.fail_write = 0;
...
// Test a successful SMTP session through a fake transport.
...
// Test SMTP sessions that encounter unexpected reply codes.
void test_session_wrong_statuses(void)
{
  const char *responses[] = {
      ...
      "220 ready\r\n250 yes\r\n250 yes\r\n250 yes\r\n500 no\r\n",
      "220 ready\r\n250 yes\r\n250 yes\r\n250 yes\r\n354 data\r\n250 yes\r\n500 no\r\n"};
  // Test each response sequence to ensure the session fails as expected.
  for (size_t index = 0; index < sizeof(responses) / sizeof(responses[0]); index++)
  {
    fake_server server = {responses[index], 0, 0, {0}, 0, 0, 0, 0};
    smtp_transport transport = fake_transport(&server);
    TEST_ASSERT_EQUAL_INT(-1, smtp_session(&transport, "a", "b", "c", "d", "e"));
  }
}

// Test an SMTP session that hangs up unexpectedly.
...
// Test reading and writing through socket functions, including error handling.
void test_socket_functions(void)
{
  ...
  TEST_ASSERT_EQUAL_INT(3, smtp_socket_read(&sockets[0], buffer, sizeof(buffer)));
  ...
  close(sockets[0]);
  close(sockets[1]);
  TEST_ASSERT_EQUAL_INT(-1, smtp_connect("invalid.invalid", "25", &(smtp_transport){0}));
  TEST_ASSERT_EQUAL_INT(-1, smtp_connect("127.0.0.1", "1", &(smtp_transport){0}));
}

// Test connecting to a real socket and then closing it.
...
// Test closing a transport and ensuring the context is freed.
